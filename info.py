...
# PyroClient Setup 
API_ID = int(environ['22562953'])
API_HASH = environ['2316a976ef770ec4db4b7657f38de1']
BOT_TOKEN = environ['7813713154:AAHWwCtFQFohfxFDv5gKOgSmww9gm9o4D-U']
...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('739891650', '').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('-1002042091363', '0').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
auth_channel = environ.get('-100207036897')
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None

# MongoDB information
DATABASE_URL = environ.get('DATABASE_URL', "mongodb+srv://Tamil222:Tamil111@cluster0.920yzub.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
